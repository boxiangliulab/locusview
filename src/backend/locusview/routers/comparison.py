...
Backs ``partials/_variant_stats.html``. It used to be the Data Browser's click-to-pin table;
click-to-pin now opens the "Search data" tab instead (``routers/search_data.py``, which reuses
:func:`_parse_dataset_keys`). The fragment stays position-based only, not rsID-based: the new
...
def _variant_rows(
    repo: QtlRepository, chrom: str, position: int, qtl_ids: list[int], gwas_ids: list[int]
) -> list[dict[str, object]]:
    """Every selected dataset's association(s) at ``chrom:position``, most significant first.
    Raises :class:`RepositoryTimeoutError` when the DB can't answer in time."""
...
def router(repo: QtlRepository) -> APIRouter:
    ...
    def variant_stats(
        ...
        """The comparison table as an HTML fragment for ``chrom``/``position`` (see module
        docstring)."""
        ...
        try:
            rows = _variant_rows(repo, chrom, position, qtl_ids, gwas_ids)
        except RepositoryTimeoutError:
            return HTMLResponse(_UNAVAILABLE_MESSAGE, status_code=503)
        title = f"chr{chrom}:{position}"
        return _render("partials/_variant_stats.html", title=title, coord=title, rows=rows)
